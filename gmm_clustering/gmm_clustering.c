...
#define MIN_CLUSTER_WEIGHT         1e-6f   // to avoid zero weights
...
static void rebalance_clusters(float* weights, float* means, float* variances,
                               float* Nk, uint32_t dim, int K, float perturb_scale)
{
    // Find largest and smallest clusters
    int max_k = 0, min_k = 0;
    for (int k = 1; k < K; ++k) {
        if (Nk[k] > Nk[max_k]) max_k = k;
        if (Nk[k] < Nk[min_k]) min_k = k;
    }

    float maxNk = Nk[max_k];
    float minNk = Nk[min_k];
    if (maxNk < 2.0f * minNk || maxNk < 10.0f) return; // skip if not strongly imbalanced or tiny sample set

    // --- Split the dominant cluster ---
    int new_k = min_k; // reuse the weakest cluster slot
    printf("[Rebalance] Splitting cluster %d (%.1f pts) into [%d,%d]; removing %d (%.1f pts)\n",
           max_k, maxNk, max_k, new_k, min_k, minNk);

    // Copy mean & variance
    memcpy(&means[new_k * dim], &means[max_k * dim], sizeof(float) * dim);
    memcpy(&variances[new_k * dim], &variances[max_k * dim], sizeof(float) * dim);

    // Perturb both means slightly in opposite directions
    for (uint32_t d = 0; d < dim; ++d) {
        float delta = perturb_scale * ((float)rand() / RAND_MAX - 0.5f) *
                      sqrtf(fmaxf(variances[max_k * dim + d], 1e-12f));
        means[max_k * dim + d] += delta;
        means[new_k * dim + d] -= delta;
    }

    // Split weights evenly
    float halfw = weights[max_k] * 0.5f;
    weights[max_k] = halfw;
    weights[new_k] = halfw;
    Nk[max_k] = Nk[new_k] = Nk[max_k] * 0.5f;
}
                  ...
                  float* out_loglike)  // out: final avg log-likelihood
{
    ...
    float var_floor = fmaxf(1e-12f, GLOBAL_VAR_FRACTION_FLOOR * fmaxf(global_var_scalar, 1e-12f));
    ...
    for (it=0; it<EM_MAX_ITERS; ++it) {
        ...
        if (it % 5 == 0) // optional: every 5 iterations
            rebalance_clusters(weights, means, variances, Nk, dim, K, 0.05f);
