...
//
// COMPILE:
//   gcc -O2 gmm_querying/remake.c 5bit_quantisation/quant_functions.c -lm -o remake
//   gcc -O3 -fopenmp gmm_query.c quant_functions.c -lm -o gmm_query
// RUN:
//  ./remake coco-i2i-512-angular 128 0.8 0.5
//  ./remake fashion-mnist-784-euclidean 128 0.8 0.5
// ------------------------------------------------------------
...
int main(int argc, char** argv) {
    ...
    snprintf(gmm_path, sizeof(gmm_path),
        "gmm_indexes/gmm/%s_train_reduced.gmm", dataset);

    snprintf(c2v_path, sizeof(c2v_path),
        "gmm_indexes/cluster_to_vectors/%s_train_reduced.index", dataset);
    ...
    char out_dir[256];
    snprintf(out_dir, sizeof(out_dir), "querying_results");
    ensure_dir("querying_results");
    ensure_dir(out_dir);

    char out_path[1024];
    snprintf(out_path, sizeof(out_path),
            "querying_results/%s_test_reduced_gmm_candidates.ivecs", dataset);
