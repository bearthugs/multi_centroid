...
def reduce_dimensionality(input_path, output_folder, target_dim=100):
    ...
    # Define base_name early so it's always available
    ...
    if target_dim >= original_dim:
        print(f"Original dimensionality ({original_dim}) <= target ({target_dim}). Skipping reduction.")
        ...
        # Skip creating the .pkl model file
        return

    print(f"Loaded {vectors.shape[0]} vectors of dimension {original_dim}")

    target_dim = min(target_dim, original_dim)
    print(f"Reducing to {target_dim} dimensions using SVD...")

    svd = TruncatedSVD(n_components=target_dim, random_state=42)
    reduced_vectors = svd.fit_transform(vectors)

    # Save SVD model only if reduction is applied
    joblib.dump(svd, os.path.join(output_folder, f"{base_name}_svd_model.pkl"))
